...
function renderMarkdown(text: string) {
  return text.split("\n").map((line, i) => {
    ...
    if (trimmed.startsWith("**") && trimmed.endsWith("**")) {
      return <h3 key={i} className="text-base font-bold text-amber-200 mt-6 mb-2">{trimmed.replace(/\*\*/g, "")}</h3>;
    }
      ...
      <div className="max-w-4xl mx-auto px-6 py-16">
        <div className="mb-10">
          <h1 className="text-4xl font-bold mb-3" style={{ color: `rgb(${accentRgb})` }}>
            Lead Magnets & Content Upgrade Generator
          </h1>
          <p className="text-slate-400 text-lg">
            Generate lead magnet ideas with content outlines and conversion copy for your audience.
          </p>
        </div>
        ...
        <form onSubmit={handleSubmit} className="space-y-5 mb-12">
          <div>
            ...
            <textarea
              ...
              placeholder="Describe your target audience: who they are, what their challenges are, what they're trying to achieve..."
              rows={4}
              className="w-full bg-gray-800 border border-gray-700 rounded-xl px-4 py-3 text-white placeholder-slate-500 focus:outline-none focus:border-amber-500 transition-colors resize-none"
              required
            />
          </div>
          <div>
            <label className="block text-sm font-medium text-slate-300 mb-2">Content Topic (optional)</label>
            <input
              type="text"
              value={topic}
              onChange={(e) => setTopic(e.target.value)}
              placeholder="What topic should the lead magnet cover?"
              className="w-full bg-gray-800 border border-gray-700 rounded-xl px-4 py-3 text-white placeholder-slate-500 focus:outline-none focus:border-amber-500 transition-colors"
            />
          </div>
          <button
            type="submit"
          ...
          >
            {loading ? "Generating Ideas..." : "Generate Lead Magnet Ideas"}
          </button>
        </form>
