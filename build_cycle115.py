#!/usr/bin/env python3
"""Build all 20 AI Psychology apps for Cycle 115"""
...
for num, name, color, title, prompt in APPS:
    app_dir = os.path.join(WORKDIR, name)
    print(f"\n{'='*60}")
    print(f"[{num}] {name}")
    print(f"{'='*60}")
    ...
    # Create Next.js app
    print(f"  [1/7] Creating Next.js app...")
    r = run(f"cd '{WORKDIR}' && npx create-next-app@latest '{name}' --typescript --tailwind --eslint --app --src-dir --import-alias '@/*' --no-turbopack --yes", capture=True)
    print(f"  {r[-300:] if len(r)>300 else r}")
    if not os.path.exists(app_dir):
        print(f"  FAIL: app not created"); continue
    ...
    <main className="min-h-screen bg-gradient-to-br from-gray-900 via-gray-950 to-gray-900 text-white flex flex-col items-center justify-center p-6">
      <div className="w-full max-w-2xl flex flex-col gap-6">
        <div className="text-center mb-2">
          <h1 className="text-3xl font-bold mb-2" style={{ color: '{color}' }}>{title}</h1>
...
"""
    src_dir = os.path.join(app_dir, "src", "app")
    with open(os.path.join(src_dir, "page.tsx"), "w") as f:
        f.write(page_tsx)

    # Write API route
    api_dir = os.path.join(src_dir, "api", "generate")
    os.makedirs(api_dir, exist_ok=True)
    route_ts = f"""import {{ NextRequest, NextResponse }} from 'next/server';
...
"""
    ...
    # Write .env.local
    with open(os.path.join(app_dir, ".env.local"), "w") as f:
        f.write(f"OPENAI_API_KEY={DEEPSEEK_KEY}\\n")

    # Install react-markdown
    print(f"  [2/7] Installing react-markdown...")
    run(f"npm install react-markdown --prefix '{app_dir}'", capture=True)

    # Build
    print(f"  [3/7] Building...")
    out = run(f"npm run build --prefix '{app_dir}'", capture=True)
    print(f"  {out[-400:] if len(out)>400 else out}")
    build_ok = os.path.exists(os.path.join(app_dir, ".next"))

    # Git init and push
    print(f"  [4/7] Git init...")
    run(f"git init", cwd=app_dir)
    run(f"git add -A", cwd=app_dir)
    run(f"git commit -m 'feat: initial commit'", cwd=app_dir)

    print(f"  [5/7] GitHub push...")
    push_out = run(f"gh repo create NebulaLumino/{name} --public --push 2>&1 || git remote add origin https://github.com/NebulaLumino/{name}.git && git push -u origin main", cwd=app_dir, capture=True)
    print(f"  {push_out[-200:] if push_out else ''}")

    # Clean up
    print(f"  [6/7] Cleaning up...")
    rmf(os.path.join(app_dir, "node_modules"))
    rmf(os.path.join(app_dir, ".next"))
    print(f"  DONE: {name} (build={'OK' if build_ok else 'FAIL'})")

print(f"\\n{'='*60}")
print("ALL 20 APPS BUILT")
print(f"{'='*60}")
