...
function renderMarkdown(text: string) {
  ...
  for (let i = 0; i < lines.length; i++) {
    ...
    if (line.startsWith('## ')) { flushUl(); elems.push(<h2 key={'h2-' + i} className="text-xl font-semibold text-emerald-400 mt-4 mb-2">{line.slice(3)}</h2>); continue; }
    ...
    <div className="min-h-screen bg-gradient-to-br from-gray-900 via-gray-950 to-gray-900 text-white p-6">
      <div className="max-w-2xl mx-auto">
        <div className="text-center mb-8">
          <h1 className="text-4xl font-bold mb-2 text-emerald-400">Emergency Fund Calculator</h1>
          <p className="text-slate-400 text-lg">Size your emergency fund based on expenses, job stability, and dependents</p>
        </div>
        <form onSubmit={handleSubmit} className="bg-slate-900/60 backdrop-blur-md border border-slate-800 rounded-2xl p-6 mb-6 space-y-4">
          <div className="flex flex-col gap-1.5">
            ...
            <textarea
              name="context"
              rows={5}
              className="bg-slate-800/80 border border-slate-700 rounded-xl px-4 py-3 text-white placeholder-slate-500 focus:outline-none focus:ring-2 focus:ring-emerald-500/50 resize-none"
          ...
          <button
            type="submit"
            disabled={loading}
            className={`w-full py-3 rounded-xl font-semibold text-white transition-all ${loading ? 'bg-slate-700/60 cursor-not-allowed opacity-60' : 'bg-emerald-600 hover:bg-emerald-500 hover:shadow-lg hover:shadow-emerald-500/20 cursor-pointer'}`}
          >
